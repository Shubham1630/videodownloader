...
public final class Tokeniser {
    ...
    public static final char[] notCharRefCharsSorted = new char[]{'\t', '\n', '\r', '\f', ' ', '<', '&'};

    // Some illegal character escapes are parsed by browsers as windows-1252 instead. See issue #1034
    // https://html.spec.whatwg.org/multipage/parsing.html#numeric-character-reference-end-state
    public static final int win1252ExtensionsStart = 0x80;
    public  static final int[] win1252Extensions = new int[] {
    ...
    public final CharacterReader reader; // html input
    public final ParseErrorList errors; // errors found while tokenising

    public TokeniserState state = TokeniserState.Data; // current tokenisation state
    public Token emitPending; // the token we are about to emit on next read
    public boolean isEmitPending = false;
    public String charsString = null; // characters pending an emit. Will fall to charsBuilder if more than one
    public StringBuilder charsBuilder = new StringBuilder(1024); // buffers characters to output as one token, if more than one emit per read
    ...
    public String lastStartTag; // the last start tag emitted, to test appropriate end tag
